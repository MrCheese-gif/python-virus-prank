...
while True:
    try:
        ...
        subprocess.run(["say", "-v", "Zarvox", phrase])
        print(phrase)
        time.sleep(PAUSE_TIME)
    except KeyboardInterrupt: # perfom exorcism
        subprocess.run(
            ["say", "-v", "Zarvox", "HE HE HE HE HE.. Evil laughter fades away.... For now"]
        )
        print("\nEvil laughter fades away.... For now")
        break
    except FileNotFoundError:
    print("'say' command not found, this software will not work if you use Linux or Windows")
    except subprocess.CalledProcessError as e: # error handling
        print(f"Error playing phrase, error is: {e}")
        subprocess.run(["say", "-v", "Zarvox", f"Error playing phrase, error is: {e}"])
        break
